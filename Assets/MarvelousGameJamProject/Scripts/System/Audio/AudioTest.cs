...
public class AudioTest : MonoBehaviour
{
    ...
    public void TestPlayBGM()
    {
        ...
        _audioManager.PlayBGM(BGMType.HalfcatBGM); // BGMType.Track1を再生（Enumで指定）
    }

    public async void TestPlayBGMWithFade()
    {
        Debug.Log("Playing first BGM with fade...");
        await _audioManager.PlayBGMWithFade(BGMType.リザルト画面BGM, 1f); // BGMType.Track1を3秒でフェードイン

        // 5秒待機して次のBGMに切り替え
        await UniTask.Delay(5000);

        Debug.Log("Switching to second BGM with fade...");
        await _audioManager.PlayBGMWithFade(BGMType.HalfcatBGM, 2f); // BGMType.Track2に2秒でフェードイン
    }
